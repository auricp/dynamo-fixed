...
import { DynamoDBClient, ListTablesCommand, DescribeTableCommand, GetItemCommand, ScanCommand, } from "@aws-sdk/client-dynamodb";
...
const DYNAMODB_DESCRIBE_TABLE_TOOL = {
...
const DYNAMODB_QUERY_TABLE_TOOL = {
...
const DYNAMODB_SCAN_TABLE_TOOL = {
...
// Utility functions
function normalizeAttributeValues(exprAttrVals) {
    if (!exprAttrVals)
        return undefined;
    ...
    Object.keys(exprAttrVals).forEach(key => {
        ...
        if (val && typeof val === "object" && (val.N || val.S || val.B)) {
            if (val.N !== undefined)
                normalized[key] = Number(val.N);
            else if (val.S !== undefined)
                normalized[key] = val.S;
            else if (val.B !== undefined)
                normalized[key] = val.B;
        }
        else {
            normalized[key] = val;
        }
...
function cleanExpressionAttributeNames(expressionAttributeNames, expressions) {
...
function fixItemKeyTypes(item, tableSchema) {
    ...
    const attrTypes = {};
    tableSchema.AttributeDefinitions.forEach((def) => {
...
async function describeTable(params) {
    try {
...
async function scanTable(params) {
    try {
        ...
        const cleanedNames = cleanExpressionAttributeNames(params.expressionAttributeNames, [params.filterExpression].filter(Boolean));
